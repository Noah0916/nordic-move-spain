...
export async function generateMetadata({ params }: VillaDetailPageProps) {
  ...
  if (!villa) {
    return {
      title: "Villa hittades inte | Nordic Move Spain",
    };
  }

  return {
    title: `Villa i ${villa.location} | ${formatPrice(villa.price)} | Nordic Move Spain`,
    description: `${villa.beds} sovrum villa i ${villa.location}. Utvald genom Nordic Move Spains bredare fastighetsnätverk.`,
  };
}

export default async function SwedishVillaDetailPage({
  params,
  ...
  const contactHref = `/sv/contact?property=${encodeURIComponent(villa.ref)}`;
    ...
    <main className="min-h-screen bg-[#f6f1ea] text-stone-900">
      <nav className="border-b border-stone-200 bg-[#1e2a3a] px-5 py-5 text-white md:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6">
          <a href="/sv" className="font-serif text-xl">
            Nordic Move Spain
          </a>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            <div className="overflow-hidden rounded-[30px] bg-stone-200 md:row-span-2">
              <img
                src={villa.images[0]}
                alt={`Villa i ${villa.location} - huvudbild`}
            ...
            <div className="grid grid-cols-2 gap-4">
              {villa.images.slice(1, 5).map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`Villa i ${villa.location} - bild ${index + 2}`}
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {villa.images.slice(5).map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`Villa i ${villa.location} - bild ${index + 6}`}
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-[34px] bg-white p-8 shadow-sm md:p-10">
              ...
              <div className="mt-8 grid grid-cols-2 gap-6 sm:grid-cols-4">
                <div>
                  <p className="text-sm text-stone-500">Sovrum</p>
                ...
                <div>
                  <p className="text-sm text-stone-500">Badrum</p>
                  <p className="mt-1 text-2xl font-medium text-[#1e2a3a]">
                    {villa.baths}
                  </p>
                </div>
                ...
                <div>
                  <p className="text-sm text-stone-500">Byggyta</p>
                  <p className="mt-1 text-2xl font-medium text-[#1e2a3a]">
                    {villa.built > 0 ? `${villa.built} m²` : "På förfrågan"}
                  </p>
                </div>

                <div>
                  <p className="text-sm text-stone-500">Tomt</p>
                  <p className="mt-1 text-2xl font-medium text-[#1e2a3a]">
                    {villa.plot > 0 ? `${villa.plot} m²` : "På förfrågan"}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-10 border-t border-stone-200 pt-8">
                <h2 className="font-serif text-3xl text-[#1e2a3a]">
                  Intresserad av den här villan?
                </h2>

                <p className="mt-5 max-w-2xl leading-relaxed text-stone-600">
                  Nordic Move Spain representerar och ger råd till köparen. Vi kan
                  kontrollera aktuell tillgänglighet, ordna en privat visning och
                  lägga till bostaden i din personliga shortlist.
                </p>

                <p className="mt-5 max-w-2xl leading-relaxed text-stone-600">
                  Före köp kan vi också samordna oberoende tekniska och juridiska
                  kontroller, så att du får en tydligare bild av vad du köper innan
                  du fattar ditt slutliga beslut.
                </p>
              </div>
            </div>
            ...
            <aside className="h-fit rounded-[34px] bg-[#1e2a3a] p-8 text-white shadow-sm md:p-10 lg:sticky lg:top-8">
              ...
              <h2 className="mt-5 font-serif text-3xl leading-tight">
                Begär en privat visning
              </h2>

              <p className="mt-5 leading-relaxed text-white/75">
                Berätta vilken bostad du är intresserad av. Vi kontaktar dig
                personligen om tillgänglighet och nästa steg.
              </p>
